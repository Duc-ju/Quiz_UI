...
function Login(props) {
  ...
  const [tab, setTab] = useState(() => {
    if (location.pathname === "/join/login") return 1;
    if (location.pathname === "/join/register") return 2;
  });
  useEffect(() => {
    if (location.pathname === "/join/login") setTab(1);
    if (location.pathname === "/join/register") setTab(2);
  }, [location.pathname]);
